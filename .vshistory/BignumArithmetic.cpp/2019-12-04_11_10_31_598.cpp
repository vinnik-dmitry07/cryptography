...
optional<LNum> babystep_giantstep(LNum g, LNum h, LNum mod) {
...
struct Curve {
	LNum p("8884933102832021670310856601112383279507496491807071433260928721853918699951");
	LNum n("8884933102832021670310856601112383279454437918059397120004264665392731659049");
	LNum a4("2481513316835306518496091950488867366805208929993787063131352719741796616329");
	LNum a6("4387305958586347890529260320831286139799795892409507048422786783411496715073");
	LNum r4("5473953786136330929505372885864126123958065998198197694258492204115618878079");
	LNum r6("5831273952509092555776116225688691072512584265972424782073602066621365105518");
	LNum gx("7638166354848741333090176068286311479365713946232310129943505521094105356372");
	LNum gy("762687367051975977761089912701686274060655281117983501949286086861823169994");
	LNum r("8094458595770206542003150089514239385761983350496862878239630488323200271273");
};
...
int main() {
	...
	LNum bits = floor_log2(n);
	LNum privatekey = LNum::random(0, (LNum(2) ^ bits) - 1);
	LNum publickey = privatekey * self.generator;
	...
	cout << decrypt(en_msg, p, key, q);
	// Baby-giant step test
	//cout << "5^x = 3 (mod 23)";
	//if (auto res = babystep_giantstep(5, 3, 23)) {
	//	cout << res.value() << '\n';
	//}
	//else {
	//	cout << "No answer!\n";
	...
	// Miller-Rabin test
	//cout << LNum("19532994432886141889218213").is_prime(5) << endl;

	// Pollards Ro test https://stackoverflow.com/a/50278321/8390594
	//LNum big("1234567891011121314151617181920212223242526272829"); //  3*859*24526282862310130729*19532994432886141889218213
	//while (big > 1) {
