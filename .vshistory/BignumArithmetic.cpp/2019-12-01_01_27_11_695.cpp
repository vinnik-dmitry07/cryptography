...
public:
	...
	static LNum random(LNum from, LNum to) {
		LNum& diff = (const LNum &)to - from;
		vector<uint64_t> parts(diff.parts.size());
		if (parts.size() > 1) {
			for_each(parts.begin(), prev(parts.end()), [](uint64_t& p) {
				p = ::random(0, base_max());
			});
			uint8_t carry = LNum(vector(diff.parts.begin(), prev(diff.parts.end()))) < LNum(vector(parts.begin(), prev(parts.end())));
			parts[parts.size() - 1] = ::random(0, diff.parts[parts.size() - 1] - carry);
			remove_zeros(parts);
			return from + LNum(parts);
		}
		else {
			return LNum(::random(from.parts[0], to.parts[0]));
		}	
	}
