...
LNum ro_pollard(LNum N) {
	LNum a = N - 2;
	LNum b = numeric_limits<uint64_t>::max();
	//LNum x = random(1, a < b ? a : b);
	return LNum(0);
...
int main() {
	cout << numeric_limits<ptrdiff_t>::max() << ' ' << vector<uint64_t>().max_size() <<
		' ' << numeric_limits<ptrdiff_t>::max() *1.0 / vector<uint64_t>().max_size() << endl;
	...
	cout << get<0>(C(LNum("34035243914635549601583369544560650254325084643201"), (LNum("10")^50) + 151));
