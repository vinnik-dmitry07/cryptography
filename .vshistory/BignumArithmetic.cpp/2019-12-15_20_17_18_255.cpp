...
private:
	...
	friend class _Default_allocator_traits<allocator<LNum>>;
	friend class tuple<const LNum, LNum>;
	friend class std::unordered_map<LNum,LNum,LNumHasher,std::equal_to<LNum>,std::allocator<std::pair<const LNum,LNum>>>;
	std::pair<std::_List_node<std::pair<const LNum, LNum>, std::_Default_allocator_traits<std::allocator<std::pair<const LNum, LNum>>>::void_pointer>*, bool> std::_Hash<std::_Umap_traits<LNum, LNum, std::_Uhash_compare<LNum, LNumHasher, std::equal_to<LNum>>, std::allocator<std::pair<const LNum, LNum>>, false>>::_Try_emplace<const LNum&>(const LNum&);
};
