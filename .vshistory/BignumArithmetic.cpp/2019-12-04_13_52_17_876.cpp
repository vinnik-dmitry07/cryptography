...
LNum babystep_giantstep(LNum g, LNum h, LNum p) {
	const auto N = LNum::sqrt(p) + 1;
	auto table = unordered_map<LNum, LNum, LNumHasher>();
	for (LNum i = 1; i <= N; i += 1) {
		table[i - 1] = g.pow_mod(i, p);
	}
	LNum c = g.pow_mod(N * (p - 2), p);
	for (LNum j = 1; j <= N; j += 1) {
		LNum y = (h * c.pow_mod(j, p)) % p;
...
int main() {
	...
	cout << "3^x = 1 (mod 196134577)";
	if (auto res = babystep_giantstep(3, 1, 196134577)) {
		cout << res.value() << '\n';
	}
	else {
		cout << "No answer!\n";
	...
	// Miller-Rabin test
	cout << LNum("18446744082299486207").is_prime(5) << endl;

	 //Pollards Ro test https://stackoverflow.com/a/50278321/8390594
	//LNum big("17348256187264213649126346457"); //  3*859*24526282862310130729*19532994432886141889218213
	//while (big > 1) {
