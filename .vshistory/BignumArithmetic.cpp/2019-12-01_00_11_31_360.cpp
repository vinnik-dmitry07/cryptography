...
public:
	...
	static LNum random(LNum from, LNum to) {
		LNum a = 0;
		vector<uint64_t> parts(to.parts.size());
		if (parts.size() > 1) {
			do {
				for_each(next(parts.begin()), parts.end(), [](uint64_t& p) {
					p = ::random(0, base_max());
					});
				parts[parts.size() - 1] = ::random(0, to.parts[parts.size() - 1]);
			} while (a > to || a < from);
		}
		else {
			parts[0] = ::random(from, to.parts[0]);
		}
		a = LNum(parts);
	}
	...
	bool is_prime(int certainty) const {
		...
		vector<uint64_t> parts(this->parts.size());
...
private:
	vector<uint64_t> remove_zeros(vector<uint64_t>& v) {
...
LNum ro_pollard(LNum n) {
	...
	uint64_t tmp = numeric_limits<uint64_t>::max();

	LNum x = random(2, n < tmp ? n.to_int() : tmp);
	LNum y = x;
	...
	LNum c = random(1, n < tmp ? n.to_int() : tmp);
	...
	while (d == 1)
	{
		...
		x = (x.pow_mod(2, n) + c + n) % n;

		/* Hare Move: y(i+1) = f(f(y(i))) */
		y = (y.pow_mod(2, n) + c + n) % n;
		y = (y.pow_mod(2, n) + c + n) % n;

		/* check gcd of |x-y| and n */
		d = gcd(LNum::abs_sub(x, y), n);

		/* retry if the algorithm fails to find prime factor
		 * with chosen x and c */
		if (d == n) return ro_pollard(n);
	}
...
int main() {
	cout << LNum("479071746608894572817856880838266287637767277").is_prime(20) << endl;
