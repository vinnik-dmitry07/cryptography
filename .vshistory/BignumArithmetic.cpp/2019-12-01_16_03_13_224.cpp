...
//	const auto m = LNum::sqrt(mod);
//	unordered_map<LNum, LNum> table = unordered_map<LNum, LNum>();
//	LNum e = 1;
...
//		auto m = uint64_t((sqrtl(5) - 1.l) * powl(2, 63));
//		for (auto& i : vec) {
...
int main() {
	auto f = sqrtl(5) - 1;
	auto x = CHAR_BIT * sizeof(uint64_t) - 1;
	/*vector<uint64_t> v = vector<int>{ 1, 2, 3 };
	cout << std::uint64_vector_hasher(v);*/
	/*cout << "5^x = 3 (mod 23)";
	cout << babystep_giantstep(5, 3, 23).value();*/
