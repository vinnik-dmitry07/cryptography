...
public:
	...
	LNum operator-(const LNum& rhs) const {
		...
		for (size_t i = 0; i < rhs.parts.size() || carry; ++i) {
			int64_t temp = lhs.parts[i];
			temp -= carry + (i < rhs.parts.size() ? rhs.parts[i] : 0);
			carry = temp < 0;
			if (carry) temp += base;
			lhs.parts[i] = temp;
		}
...
int main() {
	...
	for (LNum n = 1; n <= 25; n += 1) {
		...
		cout << "n=" << n << "\ttotient(n)=" << totient(n)  << (totient(n) == n - 1 ? "\tprime" : "");
	}

	for (LNum i = 100; i <= 100000; i *= 10) {
		LNum res(0);
		for (LNum j = 1; j <= i; j += 1) {
			if (totient1(j) + 1 == j) res += 1;
		}
		cout << res << " primes below " << i;
	}
