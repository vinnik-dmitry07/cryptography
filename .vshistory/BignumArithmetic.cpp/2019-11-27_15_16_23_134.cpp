...
LNum legendre1(LNum a, LNum p) {
	return a.pow_mod((p - 1) / 2, p);
}

// https://en.wikipedia.org/wiki/Jacobi_symbol
int16_t jacobi(LNum n, LNum k) {
	assert(n % 2 == 1);
	n %= k;
	int8_t res = 1;
	while (n != 0) {
		while (n % 2 == 0) {
			n /= 2;
			LNum r = k % 8;
			if (r == 3 || r == 5) res = -res; // (2a | n) = -(a|n) if n = 3, 5 (mod 8)
		}
		swap(n, k);
		if (n % 4 == 3 && k % 4 == 3) 
			res = -res; // (m | n) * (n | m) = -1 
...
tuple<LNum, LNum, bool> C(LNum n, LNum p) {
	auto x = legendre1(n, p);
	auto y = legendre2(n, p);
...
int main() {
	cout << legendre2(3, 1009) << endl;
	return 0;
	//for (auto f : factorization(LNum(12345))) {
	//	cout << f << ' ';
	//}
	...
	cout << jacobi(13, 9907) << endl;
	cout << jacobi(1001, 9907) << endl;

	cout << get<0>(C(LNum("881398088036"), LNum("1000000000039")));
	//.Count(x = > Totient(x) + 1 == x) :n0} primes below{ i:n0 }
	//4203
