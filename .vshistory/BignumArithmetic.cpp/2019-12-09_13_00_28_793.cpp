...
vector<LNum> ro_pollard(LNum n) {
	...
	while (!n.is_prime(5)) {
		...
		do {
			...
			do {
				x = (x * x + 1) % n;
				factor = gcd(LNum::abs_sub(x, x_fixed), n);
				count -= 1;
...
LNum totient(LNum n) {
	...
	auto tmp = ro_pollard(n);
	auto tmp1 = reduce(std::execution::seq, tmp.begin(), tmp.end(), LNum(1), [](LNum a, LNum b) { return a * b; });
	assert(n == tmp1);

	for (LNum& factor : ro_pollard(n)) {
		res -= res / factor;
	}
	return res;
}
