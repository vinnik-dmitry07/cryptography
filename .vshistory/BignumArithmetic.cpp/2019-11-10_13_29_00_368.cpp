...
class LNum {
	...
	const uint64_t base = std::pow(10, 3);
	uint64_t N, P;
public:
	LNum(string str) {
		while (str.length() >= 3) {
			parts.push_back(stoull(str.substr(str.length() - 3, 3)));
	...
	operator uint64_t() const { return stoull(this->to_str()); }
	...
	LNum operator *(const LNum& rhs) const {
		...
		for (size_t i = 0; i < lhs.parts.size(); ++i)
			for (uint64_t j = 0, carry = 0; j < (int)rhs.parts.size() || carry; ++j) {
	...
	LNum& mul(uint64_t rhs) {
		uint64_t carry = 0;
		for (size_t i = 0; i < parts.size() || carry; ++i) {
			if (i == parts.size())
				parts.push_back(0);
			uint64_t cur = carry + parts[i] * 1ll * rhs;
			parts[i] = uint64_t(cur % base);
			carry = uint64_t(cur / base);
		}
	...
	string to_str() const {
		...
		for (int i = (int)parts.size() - 2; i >= 0; --i) {
			snprintf(buff, sizeof(buff), "%03d", parts[i]);
			res += buff;
		}
		return res;
	}
...
ostream& operator<<(ostream& os, const LNum& dt)
{
	os << dt.to_str();
	return os;
}
