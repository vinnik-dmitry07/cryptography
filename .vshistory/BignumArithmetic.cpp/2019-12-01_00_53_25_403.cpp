...
public:
	...
	static LNum random(LNum from, LNum to) {
		...
		if (parts.size() > 1) {
			...
			uint8_t carry = LNum(vector(to.parts.begin(), prev(to.parts.end()))) < LNum(vector(parts.begin(), prev(parts.end())));
			parts[parts.size() - 1] = ::random(0, to.parts[parts.size() - 1] - carry);
		}
		else {
			parts[0] = ::random(from.parts[0], to.parts[0]);
		}
		remove_zeros(parts);
		return LNum(parts);
	}
