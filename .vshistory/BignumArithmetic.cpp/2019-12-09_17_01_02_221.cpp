...
LNum _ro_pollard(LNum n) {
	...
	do {
		...
		y = f(y, n, inc);
		y = f(y, n, inc);
		d = (x == y || LNum::abs_sub(x, y) == n) ? 0 : gcd(LNum::abs_sub(x, y), n);
...
vector<LNum> ro_pollard(LNum n) {
	...
	while (!n.is_prime(5)) {
		LNum tmp = _ro_pollard(n);
		if (tmp.is_prime(5)) {
			res.push_back(tmp);
			n /= tmp;
		} 
		else {
			auto add1 = ro_pollard(tmp);
			auto add2 = ro_pollard(n / tmp);
			res.insert(res.end(), add1.begin(), add1.end());
			res.insert(res.end(), add2.begin(), add2.end());
			break;
		}
	};
	res.push_back(n);
...
LNum totient(LNum n) {
	if (n == 63) __debugbreak();
	if (n == 1) return 1;
	auto factors = ro_pollard(n);
	sort(factors.begin(), factors.end());
	factors.erase(unique(factors.begin(), factors.end()), factors.end());
	for (LNum& factor : factors) {
		n -= n / factor;
	}
	return n;
}
...
int main() {
	...
	// Euler function test
	cout << totient(4032) << endl;
	//for (LNum n = 1; n <= 100; n += 1) {
