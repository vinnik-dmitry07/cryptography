...
public:
	...
	string to_str() const {
		...
		for (ptrdiff_t i = parts.size() - 2; i >= 0; --i) {
			char format[8] = "%0";
			char s[3];
			snprintf(s, sizeof(s), "%d", 12);
			strcat_s(format, sizeof(format)+sizeof(s), s);
			strcat_s(format, sizeof(format), "llu");
...
int main() {
	cout << LNum("12345678912345");
	cout << LNum(7).IsProbablePrime(20) << endl;
